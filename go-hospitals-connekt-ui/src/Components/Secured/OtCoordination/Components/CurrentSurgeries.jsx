...
const CurrentSurgeries = () => {
    ...
    const fetchCurrentSurgeries = async () => {
        
        try {
            
            const response = await axios.get(`${goHospitalsAPIBaseURL}/api/v1/ot-coordination/fetchAllCurrentSurgeries/${page}/${pageSize}`, {
                headers: {
    ...
    const fetchCurrentSurgeries2 = async (page) => {
    ...
    const nextPage = async () => {
        ...
        if ( !isLastPage ) {
            ...
            const hasPage = await fetchCurrentSurgeries2(pageNumber + 1);
...
export default CurrentSurgeries